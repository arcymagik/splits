...
% S³owa kluczowe:
\keywords{sztuczna inteligencja, gry, Splits, alfabeta, monte carlo, monte carlo tree search}
...
\begin{abstract}
  ...
  tablica transpozycji czy rozpatrywanie najpierw najbardziej obiecuj±cego syna.
...
prze³omem w~dziedzinie (\cite{rc08}). Wcze¶niej nie istnia³y ¿adne znane algorytmy pozwalaj±ce komputerom zbli¿yæ
...
implementacji stworzonych specjalnie w~tym celu. Kod wszystkich wykorzystanych programów znajduje siê
...
\emph{P³ytka} w~grze do pojedynczy element, z~którego budowana jest plansza. Jest ona podzielona na cztery sze¶ciok±ty foremne --- tak zwane
\emph{heksy}. P³ytkê przedstawiono poni¿ej na rysunku~\ref{fig:plytka}.
...
\emph{¯etony} to ma³e obiekty, których stos mo¿na umie¶ciæ na heksie.
...
oraz rozpatrywanie najpierw najbardziej obiecuj±cego syna.
Nastêpnie opisano techniki Monte Carlo, od prostego wykonywania symulacji
...
nazywaæ bêdziemy \emph{funkcj± oceniaj±c±}, mo¿emy zbadaæ drzewo gry do pewnej ustalonej g³êboko¶ci i~zdecydowaæ, który ruch
najbardziej siê nam op³aci, zak³adaj±c, ¿e przeciwnik bêdzie siê stara³ graæ tak, ¿eby wynik by³ jak najgorszy dla nas.

Sytuacje, czyli inaczej wêz³y w~drzewie gry nazywamy \emph{typu max}, je¿eli maksymalizujemy w~nich warto¶ci funkcji oceniaj±cej
potomków (czyli s± to pozycje, w~których ,,my'' wykonujemy ruch), a~te w~których minimalizujemy, nazywamy \emph{typu min} (rusza siê
przeciwnik).
...
\begin{algorithmic}[1]
  ...
  \Function{Minimaks}{\textit{node}}
  \If {\texttt{$node$ jest li¶ciem}} \Return \Call{Ocena}{\textit{node}}
  \EndIf
  \If {\texttt{$node$ jest typu max}} \Return $\max$(\Call{Minimaks}{\textit{son}} | \texttt{$son$ jest synem $node$})
  \Else {} \Return $\min$(\Call{Minimaks}{\textit{son}} | \texttt{$son$ jest synem $node$})
  \EndIf
  \EndFunction
...
to znaczy znajduje siê poza pewnym przedzia³em. Ten przedzia³ nazywa siê \emph{oknem wywo³ania}. Bêdziemy ¿±daæ, aby
...
przerwaæ obliczenia zwracaj±c tê warto¶æ jako wynik. Analogicznie postêpujemy w~wêz³ach typu min, gdy warto¶æ syna
jest mniejsza od okna. Takie przerwanie obliczeñ nazywamy \emph{ciêciem}.

Wprowadzenie oszczêdno¶ci tego typu do algorytmu minimaks nazywa siê algorytmem \emph{alfabeta}. Wraz z~ró¿nymi usprawnieniami
zosta³ dok³adnie opisany w~\cite{ap96} oraz \cite{km75}. Tutaj pseudokod przedstawiono w~algorytmie~\ref{alg:alphabeta}.
...
\label{alg:alphabeta}
\begin{algorithmic}
  \Function {Alphabeta} {\textit{node}, $\alpha$, $\beta$}
  \If {$\texttt{$node$ jest li¶ciem}$}
  \Return \Call{Ocena}{\textit{node}}
  \EndIf

  \If {$\texttt{$node$ jest typu max}$}
  \State $\textit{result} \gets -\infty$
  \ForAll {$\texttt{$s$ - syn $node$}$}
  \State $\textit{tmp} \gets \textproc{Alphabeta} (\textit{s}, \max(\textit{result}, \alpha), \beta)$
  \If {$\textit{tmp} \geq \beta$} \Return $\textit{tmp}$ \EndIf
  \State $\textit{result} \gets \max(\textit{result}, \textit{tmp})$
  \EndFor

  \Else
  \State $\textit{result} \gets +\infty$
  \ForAll {$\texttt{$s$ - syn $node$}$}
  \State $\textit{tmp} \gets \textproc{Alphabeta}(\textit{s}, \alpha, \min(\textit{result}, \beta))$
  \If {$\textit{tmp} \leq \alpha$} \Return $\textit{tmp}$ \EndIf
  \State $\textit{result} \gets \min(\textit{result}, \textit{tmp})$
  \EndFor
  \EndIf
  \Return $\textit{result}$
  \EndFunction
...
w drzewie. W~takim razie warto¶æ minimaksowa obydwu wêz³ów musi byæ taka sama. Tak± parê sytuacji nazywamy \emph{transpozycj±}.
...
\emph{Tablica transpozycji} jest s³ownikiem indeksowanym stanami gry o~warto¶ciach bêd±cych
krotkami zawieraj±cymi:
\begin{itemize}
\item obliczon± poprzednim razem warto¶æ minimaksow±,
\item informacj± czy jest~to~dok³adna warto¶æ, czy tylko ograniczenie górne lub dolne,
\item g³êboko¶æ przeszukiwania, z~którego pochodzi warto¶æ,
\item ewentualnie jakie¶ dodatkowe przydatne dane.
\end{itemize}
%%  obliczon± poprzednim razem
%% warto¶æ minimaksow±, informacj± czy jest~to~dok³adna warto¶æ, czy tylko ograniczenie górne lub dolne oraz g³êboko¶æ
%% przeszukiwania, z~którego pochodzi warto¶æ oraz ewentualnie jakie¶ dodatkowe przydatne dane.
Z~warto¶ci w~tablicy
...
Pocz±tkowo trzymamy w~tablicy jakie¶ umowne warto¶ci oznaczaj±ce niezainicjowany stan, a~gdy uda nam siê obliczyæ warto¶æ
...
\begin{algorithmic}[1]
  \Function{Iterative alphabeta} {\textit{node}}
...
\subsection{Rozpatrywanie najpierw najbardziej obiecuj±cego syna}
...
w grach, ale równie¿ w~statystyce czy rachunku ró¿niczkowym (zob. \cite{la87}). Najpro¶ciej rzecz ujmuj±c, metoda ta polega
na wykonywaniu du¿ej liczby losowych symulacji i~wyci±ganiu wniosków z~wyników.

W przypadku gry symulacj± jest losowa rozgrywka, a~wynikiem --- numer gracza zwyciêskiego. Je¿eli z~danej sytuacji
przeprowadzimy du¿o losowych rozgrywek i~wiêkszo¶æ z~nich wygra³ gracz 0, to mo¿emy wnioskowaæ, ¿e jest to stosunkowo dobra sytuacja
...
\label{alg:mc_simple}
\begin{algorithmic}[1]
  \Function{monteCarlo} {\textit{node}}
  \While {$\texttt{wci±¿ jest czas}$}
  \State $\textit{son} \gets \Call{chooseSon}{\textit{node}}$
  \State $\Call{simulate}{son}$
  \EndWhile
  \Return $\texttt{syn maksymalizuj±cy warto¶æ} $ $\frac{\texttt{ilo¶æ zwyciêstw}}{\texttt{ilo¶æ wszystkich gier}}$
...
rozgrywka.
Funkcja $\Call{chooseSon}$ wybiera po prostu losowego syna, natomiast $\Call{simulate}$
...
\begin{algorithm}
  ...
  \label{alg:mc}
  \begin{algorithmic}
    \Function {ConfidencyBound} {\textit{node}, \textit{son}}
    \State $\textit{estimated} \gets \frac{\texttt{wygrane z}~\textit{son}}{\texttt{wszystkie rozgrywki } \textit{son}}$
    \State $\sigma \gets \sqrt{\frac{\log({\texttt{wszystkie rozgrywki } \textit{node}})}{\texttt{wszystkie rozgrywki } \textit{son}}}$
    \If {\textit{node} \texttt{ jest typu max}} \Return $\textit{estimated} + \sigma$
    \Else {} \Return $\textit{estimated} - \sigma$
    \EndIf
    \EndFunction

    \Function {chooseSon} {\textit{node}}
...
By³ on z~sukcesem stosowany w~grach takich jak go, o czym mo¿na przeczytaæ w \cite{rc08}, ale równie¿
wielu innych jak na przyk³ad poker, skat, amazons, czy gry czasu rzeczywistego \cite{mcts12}.
Szczególnie go jest ¶wietnym przyk³adem
zastosowania
...
\emph{rozwiniêcia}, czyli dodajemy jego synów, którzy bêd± na razie li¶æmi drzewa. Maj±c jakie¶ drzewo przeprowadzamy
symulacjê w~sposób nieco bardziej skomplikowany ni¿ w~prostym Monte Carlo. Je¿eli znajdujemy siê w~wê¼le
nie bêd±cym li¶ciem, wchodzimy do syna wybranego metod± granicy ufno¶ci. Dochodz±c do li¶cia
...
\begin{algorithm}
  ...
  \label{alg:mcts}
  \begin{algorithmic}
    \Function {findBestMove} {\textit{node}}
    \State $\texttt{utwórz korzeñ drzewa ze stanem $node$}$
    \State $\texttt{rozwiñ drzewo w $node$}$
    \While {$\texttt{wci±¿ jest czas}$}
    \State $\Call{play}{node}$
    \EndWhile
    \Return $\texttt{syn $s$ wêz³a $node$ optymalizuj±cy warto¶æ } \frac{\texttt{wygrane z }\textit{s}}{\texttt{rozgrywki z }\textit{s}}$
    \EndFunction
    ...
    \If {$\texttt{przeprowadzono wystarczaj±co du¿o rozgrywek z $node$}$}
    \State $\texttt{rozwiñ $node$}$
...
wielu s³abych synów, ale jednego dobrego, to zostanie przez Monte Carlo uznane za s³abego syna, poniewa¿
w losowej rozgrywce czê¶ciej bêdzie wybierane które¶ z~gorszych posuniêæ. Je¿eli ju¿ jednak rozwiniemy ten wêze³,
to MCTS zacznie
...
w przeciwieñstwie do stosowania funkcji oceniaj±cej. Jest to równie¿ jeden z prostszych algorytmów, co
t³umaczy jego rosn±c± popularno¶æ.
...
\hline
alfabeta z tablic± transpozycji & \multirow{3}{*}{77} \\
i wybieraniem najpierw najbardziej & \\
obiecuj±cego syna (I funkcja) & \\
\hline
...
\hline
alfabeta z tablic± transpozycji & \multirow{3}{*}{217} \\
i wybieraniem najpierw najbardziej & \\
obiecuj±cego syna (II funkcja) & \\
\hline
\end{tabular}
\end{center}
\label{tab:ab_speeds}
...
ilo¶æ transpozycji, jakie wystêpuj± w~Splits. Rozpatrywanie najpierw najbardziej obiecuj±cego syna okazuje siê bardzo dobrym pomys³em,
...
i rozpatrywanie najbardziej obiecuj±cego syna najpierw jest niewiele lepsze ni¿ wybieranie losowego.
...
Najlepszym sposobem sprawdzenia si³y algorytmu jest przetestowanie go podczas prawdziwych rozgrywek z~innymi algorytmami.
...
w sumie 80 meczów. W~tabeli~\ref{tab:ab_tournament} podano ilo¶æ meczów zwyciê¿onych przez dany algorytm.
...
(I funkcja oceniaj±ca)&\\
\hline
alfabeta z tablic± transpozycji & \multirow{3}{*}{36} \\
i rozpatrywaniem najpierw najbardziej & \\
obiecuj±cego syna (I funkcja) &\\
\hline
...
\hline
alfabeta z tablic± transpozycji & \multirow{3}{*}{65} \\
i rozpatrywaniem najpierw najbardziej & \\
obiecuj±cego syna (II funkcja) & \\
\hline
\end{tabular}
...
Zgodnie z~przewidywaniami algorytm losowy nie odniós³ wielkiego sukcesu. Widoczna jest du¿a przewaga u¿ycia lepszej
...
przeprowadzania testowych rozgrywek, dlatego przejdziemy od razu do analizy algorytmu MCTS.
...
Próbowanie ró¿nych sta³ych nie da³o zbyt dobrych rezultatów --- wyniki algorytmów wci±¿ by³y bardzo zbli¿one.
...
z~najlepszym wariantem alfabety --- z~II funkcj± oceniaj±c±, tablic± transpozycji i~rozpatrywaniem najpierw najbardziej
obiecuj±cego
syna.
...
na razie okaza³o siê, ¿e MCTS jest zdecydowanie najlepszym algorytmem graj±cym w~Splits.
...
si³a nie jest wystarczaj±co dobrze rozpoznawana przez prost± funkcjê oceniaj±c±.
