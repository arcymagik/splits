...
\begin{abstract}
  W pracy zaimplementowano oraz przetestowano metody sztucznej inteligencji w grach na przyk³adzie gry planszowej Splits.
...
Gry towarzysz± cz³owiekowi od pocz±tków cywilizacji. S³u¿a nie tylko rozrywce, ale równie¿ samorozwojowi.
...
zwyciê¿y³a z mistrzem ¶wiata Garry'm Kasparovem.
...
u¿ytych technik sztucznej inteligencji zawarto w rozdziale 2. Opisano w nim algorytm minimax i jego pochodn± - alfabetê
oraz techniki monte carlo.
Rozdzia³ 3 przedstawia wyniki eksperymentów, w¶ród których
...
¯eton to ma³y obiekt, których stos mo¿na umie¶ciæ na heksie. W grze jest 16 ¿etonów czarnych (dla gracza 0) oraz 16 ¿etonów bia³ych
(dla gracza 1).
...
opisano klasyczny algorytm minimax oraz pochodz±c± od niego alfabetê.
...
Dok³adny opis algorytmu minimax znajduje siê w pracy doktorskiej Aske Plaata \cite{ap96}. Pseudokod przedstawiono jako
Algorytm \ref{alg:minimax}.
...
do wêz³a, nie interesuje nas dok³adna warto¶æ funkcji oceniaj±cej, je¿eli tylko wiemy, ¿e jest bardzo du¿a lub bardzo ma³a,
...
\begin{algorithmic}
  ...
  \ForAll {$\texttt{s - syn \textit{node}}$}
  \State $\textit{tmp} \gets \textit{Alphabeta} (s, max(\textit{result}, \alpha), \beta)$
  ...
  \State $\textit{result} \gets +\infty$
  \ForAll {$\texttt{s - syn \textit{node}}$}
  \State $\textit{tmp} \gets \textit{Alphabeta} (s, \alpha, min(\textit{result}, \beta))$
  \If {$\textit{tmp} \leq \beta$} \Return $\textit{tmp}$ \EndIf
...
Zauwa¿my, ¿e ilo¶æ czasu oszczêdzonego czasu, który zyskujemy u¿ywaj±c alfabety w stosunku do minimaxa, zale¿y od ilo¶ci
...
znale¼æ w \cite{mcts12}. Tutaj przedstawiono pseudokod w Algorithm~\ref{alg:mc}.
...
Algorytm przeszukiwania drzewa Monte Carlo to jeden z najsilniejszych znanych algorytmów sztucznej inteligencji w grach.
By³ on z sukcesem stosowany w grach takich jak go czy szachy. Szczególnie go jest ¶wietnym przyk³adem wysokiej jako¶ci
...
W tym punkcie przyjrzymy siê wynikom osi±ganym przez algorytm minimax oraz jego warianty. Najpierw zaprezentujemy
...
Poni¿ej w tabeli \ref{fig:ab_speeds} zaprezentowano czasy potrzebne na ca³kowite przej¶cie drzewa gry do g³êboko¶ci 5,
zaczynaj±c od pewnej typowej sytuacji po zbudowaniu planszy.

\begin{figure}
\caption{Czas potrzebny na znalezienie warto¶ci minimax drzewa o g³êboko¶ci 5}
\begin{center}
\begin {tabular}{|c|c|}
\hline
Algorytm & czas [ms] \\
\hline
minimax (I funkcja oceniaj±ca) & 32514 \\
\hline
alfabeta (I funkcja oceniaj±ca) & 55 \\
\hline
alfabeta z tablic± transpozycji (I funkcja oceniaj±ca) & 55 \\
\hline
alfabeta z tablic± i wybieraniem najlepszego syna (I funkcja oceniaj±ca) & 77 \\
\hline
minimax (II funkcja oceniaj±ca) & 33774 \\
\hline
alfabeta (II funkcja oceniaj±ca) & 977 \\
\hline
alfabeta z tablic± transpozycji (II funkcja oceniaj±ca) & 1075 \\
\hline
alfabeta z tablic± i wybieraniem najlepszego syna (II funkcja oceniaj±ca) & 217 \\
\hline
\end{tabular}
\end{center}
\label{fig:ab_speeds}
...
Rzuca siê w oczy przede wszystkim niesamowity skok wydajno¶ci przez ulepszenie do alfabety - jest przyspieszenie
...
Rzuca siê w oczy równie¿ znaczne spowolnienie wywo³ane przez u¿ycie drugiej funkcji oceniaj±cej. Przyczyna tego jest bardzo prosta.
Pierwsza funkcja nie rozró¿nia³a ruchów wystarczaj±co, a taka sama warto¶æ funkcji ju¿ powoduje wykonanie ciêcia, dlatego
przy u¿yciu pierwszej funkcji wykonuje siê du¿o wiêkszej liczby ciêæ.
Pamiêtajmy jednak, ¿e sam szybko¶æ nie przes±dza o jako¶ci funkcji oceniaj±cej. W koñcu zyskujemy lepsz± ocenê sytuacji.

\subsection{Turniej}

Najlepszym sposobem sprawdzenia si³y algorytmu jest prztestowanie go podczas prawdziwych rozgrywek z innymi algorytmami.
Na rysunku \ref{fig:ab_tournament} przedstawiono wyniki turnieju miêdzy algorytmami z funkcj± oceniaj±c± oraz algorytmem wykonuj±cym
losowe ruchy.
Turniej
...
losowy & 10 \\
\hline
minimax (I funkcja oceniaj±ca) & 22 \\
\hline
alfabeta (I funkcja oceniaj±ca) & 30 \\
\hline
alfabeta z tablic± transpozycji (I funkcja oceniaj±ca) & 32 \\
\hline
alfabeta z tablic± i wybieraniem najlepszego syna (I funkcja) & 35 \\
\hline
minimax (II funkcja oceniaj±ca) & 42 \\
\hline
alfabeta z tablic± transpozycji (II funkcja oceniaj±ca) & 59 \\
\hline
alfabeta (II funkcja oceniaj±ca) & 65 \\
\hline
alfabeta z tablic± i wybieraniem najlepszego syna (II funkcja) & 65 \\
\hline
\end{tabular}
\end{center}
\end{figure}
...
funkcji oceniaj±cej. Nawet pomimo du¿o mniejszej szybko¶ci przeszukania drzewa przez podstawowy algorytm minimax,
...
jak wielkie znaczenie ma wybór funkcji oceniaj±cej dla tej rodziny algorytmów. Bez dobrze funkcji faktycznie dobrze oddaj±cej
si³ê danej sytuacji, algorytmy minimax i alfabeta s± praktycznie bezu¿yteczne.

\section{Przeszukiwanie drzewa Monte Carlo}

W przypadku technik Monte Carlo algorytm z drzewem zdecydowanie górowa³ nad ``prostym`` Monte Carlo w czasie
przeprowadzania testowych rozgrywek, dlatego przejdziemy od razu analizy algorymu MCTS.


W algorytmie MCTS zwykle bardzo wa¿nym parametrem jest próg rozwijania wierzcho³ka drzewa, to znaczy ilo¶æ
symulacji, po których nale¿y rozwijaæ. W tabeli \ref{fig:mc_simple} przedstawiono wyniki turnieju miêdzy algorytmami
...
Okazuje siê, ¿e sta³a rozwijania nie ma wcale a¿ takieo znaczenia. Wszystkie algorytmy prezentuj± zbli¿ony poziom.
...
znakomicie. Równie¿ w starciu z alfabet± nie mia³y ¿adnych problemów.
...
dla gry planszowej Splits oraz eksperymenty sprawdzaj±ce ich efektywno¶æ w grze. Przebadano algorytmy minimax, alfabetê
...
Niektóre napotkane problemy by³y szczególnie subtelne. W algorytmie alfabeta (Algorytm \ref{alg:alfabeta})
...
skomplikowanej funkcji oceniaj±cej zajmie wiêcej czasu.
